...
pnad_com_pesos <- pnad_com_pesos %>% mutate(one = 1,
                                            Capital = case_when(CAPITAL == 11 ~ "Porto Velho (RO)",
                                                                CAPITAL == 12 ~ "Rio Branco (AC)",
                                                                CAPITAL == 13 ~ "Manaus (AM)",
                                                                CAPITAL == 14 ~ "Boa Vista (RR)",
                                                                CAPITAL == 15 ~ "Belém (PA)",
                                                                CAPITAL == 16 ~ "Macapá (AP)",
                                                                CAPITAL == 17 ~ "Palmas (TO)",
                                                                CAPITAL == 21 ~ "São Luís (MA)",
                                                                CAPITAL == 22 ~ "Teresina (PI)",
                                                                CAPITAL == 23 ~ "Fortaleza (CE)",
                                                                CAPITAL == 24 ~ "Natal (RN)",
                                                                CAPITAL == 25 ~ "João Pessoa (PB)",
                                                                CAPITAL == 26 ~ "Recife (PE)",
                                                                CAPITAL == 27 ~ "Maceió (AL)",
                                                                CAPITAL == 28 ~ "Aracaju (SE)",
                                                                CAPITAL == 29 ~ "Salvador (BA)",
                                                                CAPITAL == 31 ~ "Belo Horizonte (MG)",
                                                                CAPITAL == 32 ~ "Vitória (ES)",
                                                                CAPITAL == 33 ~ "Rio de Janeiro (RJ)",
                                                                CAPITAL == 35 ~ "São Paulo (SP)",
                                                                CAPITAL == 41 ~ "Curitiba (PR)",
                                                                CAPITAL == 42 ~ "Florianópolis (SC)",
                                                                CAPITAL == 43 ~ "Porto Alegre (RS)",
                                                                CAPITAL == 50 ~ "Campo Grande (MS)",
                                                                CAPITAL == 51 ~ "Cuiabá (MT)",
                                                                CAPITAL == 52 ~ "Goiânia (GO)",
                                                                CAPITAL == 53 ~ "Brasília (DF)"),
...
funcao_home_edu_cor <- function(municipio) {
  ...
  home_edu_cor_agrup <- ggplot(home_edu_cor, aes(fill = Escolaridade, y = ValuePer, x = Cor)) +
    ...
    labs(x = "Cor/Raça",y = "Percentual (%)", fill = "Escolaridade: ", caption = "Fonte: Microdados da PNAD Covid-19 - IBGE. Setembro, 2020.",
         title = paste0("Pessoas em home office, por cor/raça e escolaridade - ", municipio)) +
...
funcao_home_sexo_idade <- function(municipio) {
  ...
  home_sexo_idade_agrup <- ggplot(home_sexo_idade, aes(fill = Idade, y = ValuePer, x = Sexo)) +
    ...
    labs(x = "Sexo", fill = "Faixa Etária: ",y="Percentual (%)", caption = "Fonte: Microdados da PNAD Covid-19 - IBGE. Setembro, 2020.",
         title = paste0("Pessoas em home office, por sexo e faixa etária - ", municipio)) +
    scale_fill_manual(values = c("#00b894","#ff7675","#0984e3","#6c5ce7","#fdcb6e")) +
    scale_y_continuous(labels = percent_format(), limits=c(0,1)) 
...
funcao_home_emprego <- function(municipio) {
  ...
  home_emprego_agrup <- ggplot(home_emprego, aes(fill = Tipo_emprego, y = ValuePer, x = Tipo_emprego)) +
    ...
    labs(x = "Tipo de Ocupação",y="Percentual (%)", caption = "Fonte: Microdados da PNAD Covid-19 - IBGE. Setembro, 2020.",
         title = paste0("Pessoas em home office, por tipo de ocupação - ", municipio)) +
...
funcao_home_renda <- function(municipio) {
  ...
  home_renda_agrup <- ggplot(home_renda, aes(fill = Faixa_salario, y = ValuePer, x = Cor)) +
    ...
    labs(x = "Cor/Raça",y="Percentual (%)", fill = "Faixa Salarial:\n(Salários mínimos) ", caption = "Fonte: Microdados da PNAD Covid-19 - IBGE. Setembro, 2020.",
         title = paste0("Pessoas em home office, por cor/raça e faixa salarial - ", municipio)) +
...
funcao_auxilio_renda <- function(municipio) {
  ...
  auxilio_renda$Faixa_salario <- factor(auxilio_renda$Faixa_salario, levels=c("Sem renda/Não declarada",
  ...
  auxilio_renda_agrup <- ggplot(auxilio_renda, aes(fill = Faixa_salario, y = ValuePer, x = Faixa_salario)) +
    ...
    labs(x = "Faixa Salarial", y="Percentual (%)", caption = "Fonte: Microdados da PNAD Covid-19 - IBGE. Setembro, 2020.",
         title = paste0("Pessoas que receberam auxílio emergencial, por renda - ", municipio)) +
  ...
  write.csv2(auxilio_renda,paste0("auxilio_renda_",municipio,".csv"), row.names = F)
...
funcao_auxilio_cor_sexo <- function(municipio) {
  ...
  auxilio_cor_sexo_agrup <- ggplot(auxilio_cor_sexo, aes(fill = Cor, y = ValuePer, x = Sexo)) +
    ...
    labs(fill = "Cor: ", x = "Sexo",y="Percentual (%)",caption = "Fonte: Microdados da PNAD Covid-19 - IBGE. Setembro, 2020.",
         title = paste0("Pessoas que receberam auxílio emergencial, por cor/raça e sexo - ", municipio)) +
...
funcao_auxilio_cor_sexo("Salvador (BA)")
funcao_auxilio_renda("Salvador (BA)")
funcao_home_edu_cor("Salvador (BA)")
funcao_home_emprego("Salvador (BA)")
funcao_home_renda("Salvador (BA)")
funcao_home_sexo_cor("Salvador (BA)")
funcao_home_sexo_idade("Salvador (BA)")
